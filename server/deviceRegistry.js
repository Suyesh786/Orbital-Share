 ...
 * @property {'desktop' | 'mobile' | 'unknown' | null} deviceType
 * @property {'macos' | 'windows' | 'linux' | 'android' | 'ios' | 'unknown' | null} platform
...
/**
 * @param {string} socketId
 * @param {{ username: string, deviceId: string, mode: 'sender' | 'receiver', deviceType?: 'desktop' | 'mobile' | 'unknown', platform?: 'macos' | 'windows' | 'linux' | 'android' | 'ios' | 'unknown' }} data
 * @returns {boolean}
 */
export function registerClient(socketId, data) {
  ...
  const { username, deviceId, mode, deviceType, platform } = data
  ...
  const updated = {
    ...
    mode,
    deviceType:
      deviceType === "desktop" || deviceType === "mobile" || deviceType === "unknown"
        ? deviceType
        : "unknown",
    platform:
      platform === "macos" ||
