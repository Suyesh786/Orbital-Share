...
function iconForDevice(device: NearbyDevice) {
  if (device.deviceType === "mobile") return Smartphone
  if (device.platform === "macos") return Laptop
  if (device.platform === "windows" || device.platform === "linux") return Monitor
  if (device.deviceType === "desktop") return Monitor
  return Monitor
}

function deviceLabel(device: NearbyDevice) {
  const platformLabel =
  ...
  const deviceTypeLabel =
    device.deviceType === "desktop"
      ...
      : device.deviceType === "mobile"
        ? "Mobile"
        : "Device"
  return [platformLabel, deviceTypeLabel, device.status]
    .filter(Boolean)
    .join(" · ")
}
