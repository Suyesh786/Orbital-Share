...
export function HomeLowerAtmosphere() {
  return (
    <div className="pointer-events-none absolute inset-x-0 bottom-0 top-[42%]" aria-hidden>
      <div className="neardrop-lower-haze absolute inset-0" />
      <div className="neardrop-lower-glow absolute bottom-[-8%] left-1/2 size-[min(640px,110vw)] rounded-full" />
      {LOWER_PARTICLES.map((p, i) => (
        <span
          key={i}
          className="neardrop-lower-particle absolute"
          style={{
            left: p.left,
            bottom: p.bottom,
