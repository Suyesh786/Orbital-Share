...
export function TransferPage() {
  ...
  const receivedFilesSaveDirectory = useTransferStore(
    selectReceivedFilesSaveDirectory
  )
      ...
      <AnimatePresence mode="wait">
        {!complete ? (
          ...
          >
            ...
            {autoSavedToDownloads && receivedFilesSaveDirectory && (
              <p className="mt-5 text-sm text-white/45">
                Saved to <span className="text-white/70">{receivedFilesSaveDirectory}</span>
              </p>
            )}

            {summaryMode === "receiver" && receivedFilesSaveError && (
